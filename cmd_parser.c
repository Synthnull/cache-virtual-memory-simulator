...
int addArgument(Arguments *argumentList, char *str) {
	if (argumentList->numArguments == argumentList->capacity) {
		...
		argumentList->arguments =
			 realloc(argumentList->arguments,
					  argumentList->capacity * sizeof(char *));
	}

	argumentList->arguments[argumentList->numArguments] = str;
	...
	while (i < argc) {
		if (isFlag(argv[i])) {
			...
			case 's':
				...
				if ((parameters->cacheSize < 8 ||
					  parameters->cacheSize > 8192) ||
			...
			case 'a':
				...
				if (parameters->associativity != 1 &&
					 ((parameters->associativity < 2 ||
					  ...
					  !checkPowerOfTwo(parameters->associativity))) {
			...
			case 'r':
				...
				if (strcmp(currentArguments->arguments[0], "rr") == 0 ||
					 strcmp(currentArguments->arguments[0], "RR") == 0) {
					parameters->replacementPolicy = RR;
				} else if (strcmp(currentArguments->arguments[0], "rnd") == 0 ||
							  strcmp(currentArguments->arguments[0], "RND") == 0) {
			...
			case 'p':
				...
				if ((parameters->physicalMemory < 128 ||
					  parameters->physicalMemory > 4096) ||
					 !checkPowerOfTwo(parameters->physicalMemory)) {
			...
			case 'u':
				...
				parameters->physicalMemoryOS =
					 atoi(currentArguments->arguments[0]);
